...
2.  **[repx-rs](./crates):** The Execution Layer. A Rust-based unified CLI (`repx`) that synchronizes the Lab to execution targets (local or SSH/SLURM) and orchestrates job execution.
...
3.  **Run:** Use `repx run` to submit the Lab to a target (e.g., your laptop or a supercomputer). The runner handles data transfer, job scheduling, and containerization.
