    ...
    let
      analysisScript = pkgs.writeText "analysis_script.py" ''
        ...
        def main():
            ...
            # 2. Find the consumer job manually
            # Structure is usually data['jobs'] -> { job_id: { ... } }
            jobs = data.get('jobs', {})
            target_job_id = None
            ...
            for jid, jdata in jobs.items():
                # Check name or pname depending on schema version, handling potential nulls
            ...
            # 3. Resolve the output path manually
            # We know stage-C-consumer output key is "data.combined_list"
            # And we know the path template is usually "$out/combined_list.txt"
            
            # Note: The metadata schema might vary slightly based on repx-nix version.
            # Assuming 'executables' -> 'main' -> 'outputs' structure
            try:
                outputs_def = target_job_data.get('executables', {}).get('main', {}).get('outputs', {})
                # Fallback for scatter-gather or simple schema variations
                if not outputs_def:
                     # Try simple output map if flat
            ...
            # Resolve "$out" to the store path
            # Structure: store_base / outputs / job_id / out / filename
            ...
            try:
                ...
                # In a real scenario we fail, but for reference we might want to be noisy
                sys.exit(1)
