...
class ResourceHints(TypedDict, total=False):
    """Resource hints for a job or executable."""
...
class ExecutableInfo(TypedDict, total=False):
    """Metadata for a single executable within a job (e.g. "main", "scatter")."""

    path: str
    inputs: list[dict[str, Any]]
    outputs: dict[str, str]
    resource_hints: ResourceHints | None
    deps: list[str]


class JobData(TypedDict, total=False):
    """Metadata for a single job, as generated by Nix and serialized to JSON."""
...
class RootMetadata(TypedDict, total=False):
    """Top-level metadata from the Nix lab build."""
...
class LabMetadata(TypedDict):
    """The complete lab metadata container."""
...
class LocalCacheResolver(ArtifactResolver):
    ...
    def __init__(self, cache_dir: str | Path = ".repx-cache"):
...
class ManifestResolver(ArtifactResolver):
    ...
    def __init__(self, job_output_map: dict[str, str | Path]):
...
class JobView:
    ...
    def __init__(self, job_id: str, experiment: Experiment):
        ...
        self._data: JobData = self._exp._get_complete_job_data(self._id)
    ...
    def dependencies(self) -> JobCollection:
        ...
        for mapping in self.input_mappings:
            if (pid := mapping.get("job_id")) and pid != "self":
                parent_ids.add(pid)
...
class JobCollection(Sequence[JobView]):
    ...
    def filter(
        self, predicate: Callable[[JobView], bool] | None = None, **kwargs
    ) -> JobCollection:
    ...
    def __getitem__(self, index: int | slice) -> JobView | JobCollection:
...
class Experiment:
    ...
    def __init__(
        ...
        _preloaded_metadata: LabMetadata | None = None,
    ):
        self.resolver = resolver or LocalCacheResolver()

        if _preloaded_metadata is not None:
            self.path = Path(".")
            self._metadata: LabMetadata = _preloaded_metadata
        elif lab_path:
        ...
        self._effective_params_cache: dict[str, dict[str, Any]] = (
            self._calculate_all_effective_params()
        )
    ...
    def from_run_metadata(
        cls, metadata_path: str | Path, store_base: str | Path
    ) -> Experiment:
        """
        ...
        synthetic_metadata: LabMetadata = {
            "root": {"generated": True},
    ...
    def _get_single_effective_params(
        self,
        job_id: str,
        all_jobs_data: dict[str, JobData],
        visiting: set[str],
    ...
    def _calculate_all_effective_params(self) -> dict[str, dict[str, Any]]:
    ...
    @property
    def effective_params(self) -> dict[str, dict[str, Any]]:
        return self._effective_params_cache

    def _get_complete_job_data(self, job_id: str) -> JobData:
        raw_data: JobData = self._metadata.get("jobs", {}).get(job_id, {}).copy()
    ...
    def get_run_for_job(self, job_id: str) -> tuple[str, RunMetadata]:
    ...
    def runs(self) -> dict[str, RunMetadata]:
